...
void CommandHandler::start() {
    ...
    while (cmd != "EOF" && std::getline(std::cin, cmd)) {
...
void CommandHandler::start(std::ifstream&fileName ){
    std::string cmd;

    while (cmd != "EOF" && std::getline(fileName, cmd)) {
        if (isStartOfBlock(cmd)) {
            if(cmd == "EOF")break;
            _brackets.push(startOfBlock);
            openLog();
...
bool CommandHandler::isStartOfBlock(const std::string &cmd) {
    return (cmd.size() == 1 && cmd == startOfBlock);
}

bool CommandHandler::isEndOfBlock(const std::string &cmd) {
    return (cmd.size() == 1 && cmd == endOfBlock);
}
